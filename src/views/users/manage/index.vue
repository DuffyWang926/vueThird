    ...
    >
      ...
      <el-form-item label="角色" prop="roleId">
        <el-select
          ...
          placeholder="请选择角色"
          filterable
          @click="addFormRef.validateField('roleId')"
        >
...
export default {
  ...
  setup() {
    ...
    const editInfo = async (id) => {
      ...
      const { data: res } = await service.post('getUserById', {
        id
      })
      const user = res.user
    ...
    const deleteUser = (id) => {
      ElMessageBox.confirm('是否删除该账户', '提示', {
        ...
        .then(async () => {
          const res = await service.post('deleteUser', {
            id
          })
          if (res.status === 0) {
      ...
      addFormRef.value.validate((valid) => {
        if (valid) {
          ElMessageBox.confirm('确定添加账户吗？', '提示', {
            ...
            .then(async () => {
              const res = await service.post('addUser', addForm)
              if (res.status === 0) {
        ...
        if (valid) {
          ElMessageBox.confirm('确定修改账户吗？', '提示', {
            ...
            .then(async () => {
              ...
              formCopy.roleId = addForm.roleId
              const res = await service.post('editUser', formCopy)
              if (res.status === 0) {
