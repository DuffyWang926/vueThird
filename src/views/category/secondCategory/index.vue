  ...
  <el-card>
    ...
    >
      <template v-slot:userbtns="scope">
        <el-button size="mini" type="primary" @click="goToThird(scope.row.id)"
          >三级类目</el-button
        >
      </template>
    </my-table>
  ...
  <el-dialog v-model="dialogVisible" width="500px" @close="handleClose">
    ...
    >
      <el-form-item label="上级分类">
        <el-input v-model="firstCategoryName" disabled></el-input>
      </el-form-item>
...
export default {
  ...
  setup() {
    const queryInfo = reactive({
      ...
      categoryName: useRoute().query.secondCategoryName || '',
      pagenum: 1,
      pagesize: 10
    })
    ...
    const getCategories = async () => {
      const { data: res } = await service.get('getSecondCategories', {
        params: queryInfo
      })
      categories.value = res.secondCategories
    ...
    const deleteCategory = async (id) => {
      const res = await service.get('deleteSecondCategory', {
        params: { id }
      })
      if (res.status === 0) {
        ElMessage({
      ...
      editOrAddFormRef.value.validate(async (valid) => {
        if (valid) {
          ...
          if (currentEditId.value != null) {
            ...
            const res = await service.post('editSecondCategoryById', data)
            if (res.status === 0) {
              ElMessage({
                type: 'success',
                message: '修改成功'
            ...
            const res = await service.post('addSecondCategory', data)
            if (res.status === 0) {
              ElMessage({
                type: 'success',
                message: '添加成功'
